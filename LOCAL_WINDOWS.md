...
名校/名组偏好位于 `reranker.prestige`：先按相关性取前 25 篇并提取全文、摘要和机构，
再对机构名单或明确作者全名命中的论文加 0.1 分（原标度 10 分），重新排序取前 20。
多次命中不叠加，缺失机构不扣分。机构按规范化词边界匹配，作者按规范化全名匹配，
缩写或未列出的别名可能漏匹配；作者命中仅代表该作者参与，不证明论文属于某实验室。
初始名单是可编辑的偏好列表，不是完整排名或质量认证，也不由模型判断声望。
现有 65%/35% 相关性混合保持不变。设 `bonus: 0` 关闭此项，恢复只处理前 20 篇。
开启后最多额外处理 5 篇全文和摘要，但仍只发送 20 篇。
...
arXiv 流程为：全部 Atom 元数据 → 摘要相关性排序 → 选取候选短名单 → 提取全文和生成摘要 → 机构/作者小幅加分 → 前 20 篇。
